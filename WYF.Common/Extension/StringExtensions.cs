...
namespace WYF
{
    public static class StringExtensions
    {
        ...
        public static string GetMD5String(this string strtmp)
        {
            using (MD5CryptoServiceProvider provider = new MD5CryptoServiceProvider())
            {
                byte[] bytes = Encoding.UTF8.GetBytes(strtmp);
                ...
                if (builder.Length <= 0x1d)
                {
                    return builder.ToString();
                }
                return builder.ToString().Substring(1, 0x1d);
            }
        }
        /// <summary>
        /// 指示指定的字符串是 null、空或者仅由空白字符组成。
